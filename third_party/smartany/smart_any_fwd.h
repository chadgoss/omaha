...
struct close_delete
{
    ...
    static void close( T * p )
    {
        ...
        static detail::static_assert_struct<0 != sizeof( T )> const cannot_delete_an_incomplete_type;

        #ifdef SMART_ANY_PTS
        // This checks to make sure we're not calling delete on a HANDLE
        static detail::static_assert_struct<!detail::is_handle<T*>::value> const cannot_delete_a_handle;
        #endif
